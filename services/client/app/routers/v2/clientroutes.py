...
@router.post("/add")
async def add(payload: CDFRequest, db: AsyncSession = Depends(get_session)):
    clients = []
    for client in payload.data:
        res = await ClientService(db).create(client)
        clients.extend(res)
    return JSONResponseBuilder() \
...
async def update(client_id: int, payload: CDFRequest, db: AsyncSession = Depends(get_session)):
    client = payload.data
    client.id = client_id
